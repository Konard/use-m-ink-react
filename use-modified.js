...
// MODIFICATION: Add React context sharing mechanism
const sharedModules = new Map();
const reactEcosystemPackages = new Set([
  'react', 'react-dom', 'react-reconciler', 'scheduler',
  'ink', '@ink/ui', 'react-ink', 'ink-spinner', 'ink-text-input'
]);

const isReactEcosystem = (moduleSpecifier) => {
  const { packageName } = parseModuleSpecifier(moduleSpecifier);
  return reactEcosystemPackages.has(packageName) || packageName.startsWith('react-') || packageName.startsWith('@react/') || packageName.includes('ink');
};

const baseUse = async (modulePath, moduleSpecifier) => {
  // MODIFICATION: Check if this is a React ecosystem package and if we already have it loaded
  if (moduleSpecifier && isReactEcosystem(moduleSpecifier)) {
    const { packageName } = parseModuleSpecifier(moduleSpecifier);
    
    // If we already have this React ecosystem package, return the shared instance
    if (sharedModules.has(packageName)) {
      console.log(`🔄 Reusing shared ${packageName} instance for React context compatibility`);
      return sharedModules.get(packageName);
    }
  }
  ...
  try {
    ...
    // MODIFICATION: Store React ecosystem packages in shared cache
    if (moduleSpecifier && isReactEcosystem(moduleSpecifier)) {
      const { packageName } = parseModuleSpecifier(moduleSpecifier);
      console.log(`✅ Caching ${packageName} for React context sharing`);
      
      // Check if the only key in the module is 'default'
      const keys = Object.keys(module);
      const exportedModule = (keys.length === 1 && keys[0] === 'default') ? (module.default || module) : module;
      
      sharedModules.set(packageName, exportedModule);
      return exportedModule;
    }
    
    // For non-React packages, use original behavior
    const keys = Object.keys(module);
...
const makeUse = async (options) => {
  ...
  return async (moduleSpecifier) => {
    ...
    return baseUse(modulePath, moduleSpecifier); // MODIFICATION: Pass moduleSpecifier to baseUse
  };
}
...
// MODIFICATION: Add method to clear shared modules cache if needed
use.clearReactCache = () => {
  console.log('🗑️ Clearing React ecosystem shared modules cache');
  sharedModules.clear();
};

// MODIFICATION: Add method to check what's in the cache
use.getSharedModules = () => {
  return Array.from(sharedModules.keys());
};
